...
export interface GridSimulationInput {
  ...
  /**
   * 买入触发价（阈值全仓策略 strategy5 专用）。当日收盘价 ≤ 此价且空仓时，用全部可用资金全仓买入。
   */
  buyThreshold?: number | null;
  /**
   * 卖出触发价（阈值全仓策略 strategy5 专用）。当日收盘价 ≥ 此价且持仓时，清仓卖出全部持仓。
   * 卖出触发价须高于买入触发价，避免同一交易日反复横跳。
   */
   ...
   *   偏差超过 ±5% 时按「目标持仓股数 − 当前持仓股数」买入/卖出（取整到 100 股），否则不操作。
   * - 'strategy5'（阈值全仓）：不依赖网格，仅用两个触发价做「全仓 / 空仓」二态切换——空仓且收盘价 ≤ 买入触发价则全仓买入，
   *   持仓且收盘价 ≥ 卖出触发价则清仓卖出；成交价取当日收盘价。
   */
  gridStrategy?: 'strategy1' | 'strategy2' | 'strategy3' | 'strategy4' | 'strategy5';
}
