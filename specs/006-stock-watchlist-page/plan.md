...
- **变更内容**: 在自选股添加弹框中，当用户输入完整的股票代码后（失去焦点或按回车键），系统应自动调用API获取并填充股票名称，无需用户手动输入。同时显示加载状态提示。编辑模式下不触发自动获取。
- **变更原因**: 提升用户体验，减少手动输入错误，与持仓页面新增交易弹框保持一致的交互方式

## Summary

实现自选股页面功能，用户可在 Electron 桌面应用中管理自选股列表、设置价格阈值、开启/关闭监控，并通过定时任务自动获取股票价格。页面展示股票代码、名称、开盘价、当日最高价、当日最低价、涨跌额、涨跌幅、当前价格等完整日内行情信息（涨跌额/涨跌幅遵循A股红涨绿跌惯例）。告警通过页面显示和系统通知方式提醒用户，数据持久化到 SQLite 数据库。
