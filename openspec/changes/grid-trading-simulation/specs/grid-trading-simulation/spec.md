...
- **FR-008**: 采用游标档位 + 虚拟卖出占位模型；维护游标 `cursorIdx`（最近一次操作档位，含虚拟卖出）。买卖 MUST 相对游标相邻档位判定：当日 `high` 涨穿 `levels[cursorIdx+1]` → 以该档位值「卖出」并游标上移一格（真实持仓 > 0 则实际卖出全部持仓，已空仓则记 `virtual` 虚拟卖出仅占位、不影响现金/持仓）；当日 `low` 跌穿 `levels[cursorIdx-1]` → 以该档位值真实买入并游标下移一格。成交价 MUST 采用档位值（非收盘价），均不得超过网格上下限；`spacingType='percentage'` 时档位按等比（上一档 × (1 + spacing)）生成
